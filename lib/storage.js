...
Storage.prototype.typeDirExists = function(type){
  return new Promise((resolve, reject) => {
    fs.stat(`${this.rootDir}/${type}`, (err, stats) => {
      if (err) {
        debug(err);
  ...
  return new Promise((resolve, reject) => {
    co((function*(){
      ...
      co((function*(){
        ...
        err = new AppError(`Storage type: unable to create type dir: ${type} `, 500, 'internal server error');
        reject(err);
      });
    });
  });
...
Storage.prototype.mkTypeDir = function(type){
  return new Promise((resolve, reject) => {
    fs.mkdir(`${this.rootDir}/${type}`, function(err){
      if (err) {
        debug(err);
  ...
  return new Promise((resolve, reject) => {
    ...
    fs.readFile(filepath, function(err, data){
      ...
      try {
        ...
        debug(err);
        err = new AppError(`could not parse item file: ${filepath}`, 500, 'internal server error');
      }
    });
  });
};
...
Storage.prototype.writeItemFile = function(type, item){
  return new Promise((resolve, reject) => {
    ...
    fs.writeFile(filepath, JSON.stringify(item), (err, data) => {
      if (err) {
        debug(err);
